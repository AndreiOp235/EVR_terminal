...
SERIAL_RESET_INTERVAL = 3.0

VALUE_MIN = 0
VALUE_MAX = 4000
UI_REFRESH_MS = 100

EXCLUDE = [(3,3), (6,5), (8,1), (9,0), (13,3)]


def parse_args():
    parser = argparse.ArgumentParser(description="Termistori Heatmap cu salvare CSV")
    parser.add_argument("X", type=str, help="Identificator sesiune; datele se salveaza in CSV-uri/TEMP_X/")
...
class CsvSaver:
    """Salveaza 100 de iteratii ale tabelului in CSV-uri/TEMP_X/"""

    MAX_ITERATIONS = 100

    def __init__(self, x_label):
        self.folder = os.path.join("CSV-uri", f"TEMP_{x_label}")
    ...
    def try_save(self, values):
        """Incearca sa salveze o iteratie. Returneaza True daca s-a salvat, False daca s-a atins limita."""
        with self._lock:
            if self._done:
                return False
            ...
            filename = os.path.join(self.folder, f"iteratie_{iteration:03d}.csv")

            with open(filename, "w", newline="") as f:
                writer = csv.writer(f)

                # Header: coloanele P0..P7
                header = ["Bank"] + [f"P{j}" for j in range(THERMISTORS_PER_BANK)]
                writer.writerow(header)

                for i, row in enumerate(values):
                    writer.writerow([f"B{i}"] + row)
...
class HeatmapApp:
    ...
    def __init__(self, root, csv_saver: CsvSaver):
        ...
        self._dirty = True
        self._new_full_frame = False  # setat True cand un cadru complet e gata de salvat
    ...
    def push_value(self, bank, thermistor, value, packets, sync_err, crc_err):
        with self._lock:
            ...
            # Un cadru complet = ultimul termistor din ultima banca primit
            if bank == THERMISTOR_BANKS - 1 and thermistor == THERMISTORS_PER_BANK - 1:
                self._new_full_frame = True
    ...
    def _ui_refresh(self):
        with self._lock:
            ...
            status = self._status
            save_now = self._new_full_frame and not self.csv_saver.done
            self._new_full_frame = False
            self._dirty = False

        # Salvare CSV in afara lock-ului
        if save_now:
            self.csv_saver.try_save(values)

        csv_count = self.csv_saver.count
        csv_done = self.csv_saver.done

        for i in range(THERMISTOR_BANKS):
            for j in range(THERMISTORS_PER_BANK):
                v = values[i][j]
                r, g, b = jet_color(v)
                self.canvas.itemconfig(self.rects[i][j], fill=rgb_to_hex(r, g, b))
        ...
        csv_info = f"CSV: {csv_count}/100 ✓" if csv_done else f"CSV: {csv_count}/100"

        self.title_var.set(
            f"Termistori live | Pachete OK: {packets} | Sync err: {sync_err} | "
            f"Media: {mean_value:.1f} | {csv_info}")
            ...
            for byte in data:
                ...
                if len(buffer) == PACKET_SIZE:
                    if is_valid(buffer):
                        ...
                        packets_received += 1
                        app.push_value(bank, term, value,
                                       packets_received, sync_errors, crc_errors)
                    else:
                        sync_errors += 1
                    buffer = []
    finally:
...
if __name__ == '__main__':
    ...
    csv_saver = CsvSaver(args.X)
    print(f"[INFO] Sesiune: {args.X} | Folder output: {csv_saver.folder}")

    root = tk.Tk()
    app = HeatmapApp(root, csv_saver)
